...
import {
  CreateCountriesInfoDto,
  CreateCountrySourceDto,
} from './dto/create-county_source.dto';
...
export class CountySourcesService {
  ...
  async bulkUpdateSource(updateBulkSourceDto: UpdateBulkSourceDto) {
    try {
      const dataArray = updateBulkSourceDto.items;

      for (const item of dataArray) {
        if (item?.id) {
          await this.countrySourcesRepo.update(
            { id: item.id } as any,
            {
              ...item,
              modified_datetime: new Date(),
          ...
          await this.countrySourcesRepo.save({
            ...item,
            datetime: new Date(),
            modified_datetime: new Date(),
            status: 'ACTIVE',
